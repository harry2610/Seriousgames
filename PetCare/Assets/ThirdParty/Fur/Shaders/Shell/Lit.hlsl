...
float4 frag(Varyings input) : SV_Target
{
    ...
    float4 furLengthColor = SAMPLE_TEXTURE2D(_FurLengthMap, sampler_FurLengthMap, input.uv);
    float alpha = furColor.r * (1.0 - input.layer);
    if ((input.layer > 0.0 && alpha < _AlphaCutout) || input.layer > furLengthColor.r)
        discard;

    float3 viewDirWS = SafeNormalize(GetCameraPositionWS() - input.positionWS);
    float3 normalTS = UnpackNormalScale(
        SAMPLE_TEXTURE2D(_NormalMap, sampler_NormalMap, furUv), 
        _NormalScale * furLengthColor.r);
    ...
    surfaceData.occlusion = lerp(1.0 - _Occlusion * furLengthColor.r, 1.0, input.layer);
    surfaceData.albedo = _DirtColor.rgb * furLengthColor.g + surfaceData.albedo * (1.0 - furLengthColor.g);
